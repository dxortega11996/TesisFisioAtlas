...
class listarSeguimientoPaciente(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['Profesional']=perfil
        print('el context es ', context['pacientes'])
        return context

def activarHC(request):
    idPaciente= request.POST.get('idPaciente')
    print('idPAciemte',idPaciente)
...
class RegistrarRutina(CreateView):
        ...
        def post(self, request, *args, **kwargs):
            ...
            if form.is_valid() :
                ...
                vRutina.save()
                idRutina=vRutina.id
                """ numSesiones= request.POST['num_sesion']
                numSecion=1
                for i in range(int(numSesiones)):
                    newControl=Control.objects.create(rutina_id=idRutina,numeroSesion=numSecion)
                    numSecion+=1
                    newControl.save() """
                ...
                if juego2=='SI':
                  for i in range(int(numsesionesJuego2)):
                    newControl=Control.objects.create(rutina_id=idRutina,numeroSesion=numSesion2,juego_id=2)
                    numSesion2+=1
                    newControl.save()
                ...
                if juego3=='SI':
                  for i in range(int(numsesionesJuego3)):
                    newControl=Control.objects.create(rutina_id=idRutina,numeroSesion=numSesion3,juego_id=3)
                    numSesion3+=1
                    newControl.save()
